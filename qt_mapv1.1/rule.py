...
class Compile(object):
    def __init__(self, chip, count_remain_gate):
        self.chip = chip
        self.discount = 0.9
        self.miu = 0.9
        self.count_remain_gate = count_remain_gate

    def start_play(self, compiler, circuit, logical_number):
        self.circuit = deepcopy(circuit)
        self.chip.init_chip(logical_number, self.circuit)
        ...
        while True:
            ...
            move = compiler.get_action(self.chip, self.remain_depth, self.whole_depth)
    ...
    def start_self_play(self, player, whole_depth, logical_number, is_shown=0, temp=1e-3):
        ...
        self.chip.init_chip(logical_number, circuit)
        ...
        while True:
            ...
            if self.remain_depth > temp_remain_depth: 
                ...
                nothing_had_done = False
                small_reward = self.count_remain_gate(self.chip.state['q'].q[0], curr_q) / (logical_number // 2)
            if self.remain_depth:
                curr_q = deepcopy(self.chip.state['q'].q[0])   
    ...
    def start_dqn_self_play(self, player, whole_depth, logical_number, is_shown=0, temp=1e-3):
        ...
        while True:
            ...
            if self.remain_depth > temp_remain_depth:
                ...
                reward = self.count_remain_gate(self.chip.state['q'].q[0], curr_q) / (logical_number // 2)
                rewards.append(reward)
                nothing_had_done = False
            else:
