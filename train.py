...
class TrainPipeline():
    def __init__(self, newnet, logical_number, circuit, count_remain_gate, init_model=None):
        ...
        self.compile = Compile(self.chip, logical_number, self.count_remain_gate)
        # training params
    ...
    def run(self, whole_depth, logical_number):
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                with open('.\\loss\\MTCS_loss_3x3_5_30', 'a') as f1:
                    for i in range(len(losses)):
                        f1.write(str(float(losses[i])))
                        f1.write(' ')
                    f1.close()
                with open('.\\loss\\MTCS_entropy_3x3_5_30', 'a') as f1:
                    for i in range(len(entropys)):
                        f1.write(str(float(entropys[i])))
                        f1.write(' ')
                    f1.close()
                with open('.\\data\\MTCS_swap_3x3_5_30', 'a') as f1:
                    for i in range(len(swaps)):
                        f1.write(str(int(swaps[i])))
                        f1.write(' ')
                    f1.close()
                with open('.\\data\\MTCS_time_3x3_5_30', 'a') as f1:
                    for i in range(len(times)):
                        f1.write(str(int(times[i])))
                        f1.write(' ')
                    f1.close()
