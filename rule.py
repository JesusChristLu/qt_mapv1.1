...
class Chip(object):
    ...
    def next_state(self, move):
        ...
        if -1 not in loc:
            newself.E[loc[0]], newself.E[loc[1]] = newself.E[loc[1]], newself.E[loc[0]]
        curr_state = newself.current_state()
        for i in range(newself.chip_size):# maybe can be changed to logical number
            if curr_state[0, 0, i]:
                for j in range(i + 1, newself.chip_size):# maybe can be changed to logical number
                    if not(curr_state[0, 0, i] + curr_state[0, 0, j]):
                        qj = np.where(newself.E == j + 1)
                        qi = np.where(newself.E == i + 1)
                        qi = int(qi[0])
                        qj = int(qj[0])
                        if newself.connection[qi, qj]:
                            curr_state[0, 0, int(i)] = curr_state[0, 0, int(j)] = 0
                            newself.state['q'].q[0] = curr_state[0, 0, 0 : len(newself.state['q'].q[0])]
                        break
        if False not in (np.zeros(newself.chip_size) == curr_state[0, 0, :]):
            newself.state['q'].q = newself.state['q'].q[1 :]
        curr_state = newself.current_state()
        return curr_state

    def compute_layers_distance(self, gamma = 0.01):
        curr_state = self.current_state()
        distance = 0
...
class Compile(object):
    def __init__(self, chip, logical_number):
        self.chip = chip
    ...
    def start_self_play(self, player, whole_depth, logical_number, is_shown=0, temp=1e-3):
        ...
        while True:
            ...
            if self.remain_depth:
                curr_q = deepcopy(self.chip.state['q'].q[0])   
            #completness.append(self.remain_depth / self.whole_depth)
            time += 1
            total_swap_number += swap_number
            completness.append(total_swap_number) 
            print('the after map: ', self.chip.state['E'])
            print('time: ', time, ', remain depth: ', self.remain_depth, 'swap number: ', swap_number)
            if not self.remain_depth:
                ...
                print('total_swap_number: ', total_swap_number)
                player.reset_player()
                return zip(states, mcts_probs, completness)
    ...
    def start_dqn_self_play(self, player, whole_depth, logical_number, is_shown=0, temp=1e-3):
        ...
        while True:
            ...
            # store the data
            states.append(self.chip.current_state())
            next_states.append(self.chip.next_state(move))
            moves.append(move)
            # perform a move
            temp_remain_depth, swap_number = self.chip.do_move(move, self.remain_depth, nothing_had_done)
            if self.remain_depth > temp_remain_depth:
                reward = 1 / (swap_number + 0.1) + 0.09 * (pre_distance - self.chip.compute_layers_distance(foresee))
                rewards.append(reward)
                self.remain_depth = temp_remain_depth
                nothing_had_done = False
            elif False in (self.chip.state['q'].q[0] == curr_q):
                reward = 1 / (swap_number + 1) + 0.09 * (pre_distance - self.chip.compute_layers_distance(foresee))
                rewards.append(reward)
                nothing_had_done = False
            else:
                reward = 0.09 * (pre_distance - self.chip.compute_layers_distance(foresee))
                rewards.append(reward)  
                nothing_had_done = True
            if self.remain_depth:
                curr_q = deepcopy(self.chip.state['q'].q[0])
            ...
            if not self.remain_depth:
                print('total_swap_number: ', total_swap_number)
                player.reset_player()
                return zip(states, moves, rewards, next_states)